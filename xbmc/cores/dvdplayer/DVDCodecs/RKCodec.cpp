  ...
  while(!m_bStop)
  {
    ...
    if (m_1st_pts > 0 || m_1st_pts == DVD_NOPTS_VALUE)
    {
      ...
      if (m_cur_pts != video_pts)
      {
        ...
        {
          CLog::Log(LOGDEBUG,"RKAdjustLatency discontinuely!");
          m_1st_adjust = m_cur_adjust;
        }
        m_last_adjust = m_cur_adjust;
        RKAdjustError = m_cur_adjust - m_1st_adjust;
        ...
        CLog::Log(LOGDEBUG,"Process master:%lld local:%lld err:%lf adjust:%lld offet:%lf",(int64_t)app_pts,video_pts, error,(m_cur_adjust - m_1st_adjust),offset);
